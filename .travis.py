...
def main():
	if which("sudo") and which("apt-get"):
		...
		if platform.machine() == "x86_64":
			call(["sudo", "apt-get", "install", "-qq", "--force-yes", "libgd2-xpm", "ia32-libs", "ia32-libs-multiarch"]);
	else:
	...
	call(["android", "update", "sdk", "--filter", "platform-tools,android-16", "--no-ui", "--force"])
...
def unsecure():
	print "Runned in unsecure mode, deploy function is disabled"
	call("mvn", "clean", "install", "--quiet", "--batch-mode", "-DskipTests=true")

def secure():	
	print "Secure mode, create new maven config"
	maven_config = os.getcwd() + "/.maven.xml"
	create_maven_config(maven_config)
	if "[deploy snapshot]" in call_output(["git", "log", "-1", "--pretty=%B", os.environ["TRAVIS_COMMIT"]]):
		call(["mvn", "clean", "install", "deploy", "--quiet", "--batch-mode", "-DskipTests=true", "--settings=" + maven_config])
	else:
		call(["mvn", "clean", "deploy", "--quiet", "--batch-mode", "-DskipTests=true", "--settings=" + maven_config])

def create_maven_config(filename):  
	...
	serverUser.appendChild(m2.createTextNode(os.environ["SONATYPE_USERNAME"]))
	serverNode.appendChild(serverUser)
	
	serverPass = m2.createElement("password") 
	serverPass.appendChild(m2.createTextNode(os.environ["SONATYPE_PASSWORD"])) 
