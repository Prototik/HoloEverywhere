...
import com.WazaBe.HoloEverywhere.app.HoloMapActivity;
...
public abstract class SMapActivity extends HoloMapActivity implements SBase {
	...
	if (isABSSupport() && featureId == Window.FEATURE_OPTIONS_PANEL && !mIgnoreNativeCreate) {
	...
	if (isABSSupport() && featureId == Window.FEATURE_OPTIONS_PANEL && !mIgnoreNativeSelected) {
	...
	if (isABSSupport() && featureId == Window.FEATURE_OPTIONS_PANEL && !mIgnoreNativePrepare) {
	...
	if (isABSSupport()) {
	    getSherlock().setContentView(prepareDecorView(getLayoutInflater().inflate(layoutResId)));
	} else {
